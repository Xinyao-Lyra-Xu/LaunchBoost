...
export function BulkImportModal({ isOpen, onClose, onImport }: BulkImportModalProps) {
  ...
  function handleConfirm() {
    ...
    lines.forEach((line, i) => {
      ...
      if (cat && !VALID_CAT.includes(cat))
        errs.push(`第 ${i + 1} 行: 分类 "${cat}" 无效，已设为 study`);
      if (diff && !VALID_DIFF.includes(diff))
        errs.push(`第 ${i + 1} 行: 难度 "${diff}" 无效，已设为 easy`);
    ...
    <div id="bulk-import-modal" className="modal">
      ...
      <div className="modal-content task-edit-content bulk-modal-content">
        ...
        <div className="modal-actions">
          <button id="bulk-cancel-btn" className="btn-secondary" onClick={onClose}>
            取消
          </button>
          <button id="bulk-confirm-btn" className="btn-primary" onClick={handleConfirm}>
            导入
          </button>
        </div>
      </div>
